    ...
    switch (msg.MsgType) {
        case bot.CONF.MSGTYPE_TEXT:
            ...
            if (text && text.length >= 2) {
                ...
                console.log(msg)
                if (text === '-功能') {
                    sendMsg.returnTxt('本V功能输入指令如下：\n搜图 识图 相似图 记数字', toMsgName, bot)
                } else if (text.slice(0, 2) === '搜图') {
                    sendMsg.sendImg(text.slice(2), toMsgName, bot)
                    console.log('搜索文字=======>' + text.slice(2));
                } else if (text.slice(0, 2) === '识图') {
                    snalysisNameArr.push(fromName);
                } else if (text.slice(0, 3) === '相似图') {
                    // sendMsg.returnTxt('暂无此功能 ^~^', toMsgName, bot);
                } else if (text.slice(0, 3) === '玩游戏') {
                    // sendMsg.returnTxt('请选择:\n记数字', toMsgName, bot);
                } else if (text.slice(0, 3) === '记数字') {
                    // sendMsg.returnTxt('请在', toMsgName, bot);
                }
            }
            break;
            ...
            if (snalysisNameArr.indexOf(msg.FromUserName) !== -1) {
                bot.getMsgImg(msg.MsgId || msg.NewMsgId).then(res => {
                    ...
                    aipImgClient.advancedGeneral(img).then(function (result) {
                        ...
                        sendMsg.returnTxt(msg, toMsgName, bot);
                    }).catch(function (err) {
                        console.log(err)
                        sendMsg.returnImgErr(bot, toMsgName, '识图')
                    })
                }).catch(err => {
                    console.log(err)
                })
